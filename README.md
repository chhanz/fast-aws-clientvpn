...
4. 접속용 `.ovpn` 파일을 생성합니다 (`$HOME/clientvpn-certs/` 아래).
...
- 인증서/키 및 `.ovpn`: `$HOME/clientvpn-certs/`
...
| 배포 후 인터넷이 안 됨 (Full Tunnel) | Full Tunnel은 모든 트래픽을 VPN으로 보냅니다. 서브넷의 NAT/IGW 경로를 확인하세요. |
